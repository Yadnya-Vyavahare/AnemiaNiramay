...
const ForgotPassword = ({ navigation }) => {
  ...
  const onSendPressed = async () => {
    ...
    if (!email ) {
      setEmailError('Enter an email ID!');
      errorCount++;
    } 
    ...
    try {
      const response = await axios.post(`${config.API_BASE_URL}/Setemail`, {
         email : email
      });
    ...
    <ScrollView showsVerticalScrollIndicator={false}>
      <View>
        ...
        <View style={styles.inputContainer}>
          ...
          <Input
            title="Using Email ID/ईमेल आयडी"
          ...
          {emailError ? <Text style={styles.errorMessage}>{emailError}</Text> : null} 
            ...
            <TouchableOpacity onPress={onPressPhone} style={[styles.button, { alignSelf: 'flex-end' }]}>
            <Text style={{ color: 'black', fontSize: 16 }}>Using Phone ?</Text>
            </TouchableOpacity>
