...
const WeightDifferenceGraph1 = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(`${config.API_BASE_URL}/BMIGraph?ID=${childFullName}`);
  ...
  const prepareChartData = () => {
    if (data.length > 0) {
        const labels = data.map((value, index) => `Visit ${index + 1}`);
          console.log(labels);
      const firstHeightData = data.map(item => item.BMI);
...
<ScrollView>
    <View style={styles.container}>
      ...
      <Text style={styles.title}>BMI Graph</Text>
      ...
      {data.length > 0 ? (
        ...
        <Text>No data available</Text>
                  ...
                  <View style={styles.table}>
                    ...
                    <Button style={{width:10, height:50}} title="CAPTURE" onPress={captureScreen} />
    </View>
    </ScrollView>
  );
};
...
export default WeightDifferenceGraph1;
