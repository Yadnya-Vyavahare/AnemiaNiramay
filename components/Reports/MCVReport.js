...
const WeightDifferenceGraph = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(`${config.API_BASE_URL}/MCVGraph?ID=${childFullName}`);
  ...
  const prepareChartData = () => {
    if (data.length > 0) {
        const labels = data.map((value, index) => `Visit ${index + 1}`);
          console.log(labels);
      const firstHeightData = data.map(item => item.MCV);
...
<ScrollView>
    <View style={styles.container}>
      ...
      <Text style={styles.title}>MCV Graph</Text>
...
<View style={styles.table}>
                        ...
                        <View style={styles.tableContainer}>
                            <View style={styles.tableHeader}>
                                ...
                                <Text style={styles.tableHeaderText}>    MCV</Text>
                            </View>
                            {data.map((item, index) => (
                                <View style={styles.tableRow} key={index}>
                                    <Text style={styles.tableCell}>{item.formatted_date}</Text>
                                    <Text style={styles.tableCell}>{item.MCV}</Text>
