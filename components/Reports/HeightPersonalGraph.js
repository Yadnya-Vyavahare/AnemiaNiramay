...
import { View, Text,Button,Dimensions, StyleSheet ,TextInput,TouchableOpacity} from 'react-native';
...
const HeightDifferenceGraph = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(`${config.API_BASE_URL}/personalHeightGraph?ID=${childFullName}`);
  ...
  const prepareChartData = () => {
    if (data.length > 0) {
      ...
      const firstHeightData = data.map(item => item.height);
  ...
  return (
    ...
    <View style={styles.container}>
      ...
      <Text style={styles.title}>Height Analysis Graph</Text>
