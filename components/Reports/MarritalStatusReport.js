...
const AgeAnalysisComponent = () => {
  ...
  const [countUnmarried,setcountUnmarried] = useState("");
  const [countOver18,setcountOver18] = useState("");
  const [countUnder18,setUnder18] = useState("");
  ...
  useEffect(() => {
    ...
    const fetchAgeData = async () => {
      try {
        ...
        response = await axios.get(`${config.API_BASE_URL}/MarriedAnalysis?year=${selectedYear}&kishoriVarg=${selectedKVarg}`);
      } else if (selectedKVarg !== null) {
        console.log('lolol')
        response = await axios.get(`${config.API_BASE_URL}/MarriedAnalysis?kishoriVarg=${selectedKVarg}`);
      } else if (selectedYear !== null) {
        response = await axios.get(`${config.API_BASE_URL}/MarriedAnalysis?year=${selectedYear}`);
      } else {
        response = await axios.get(`${config.API_BASE_URL}/MarriedAnalysis`);
      }
      ...
      if (!response || !response.data) {
        ...
        setUnder18(response.data[0].count_under18);
        setcountOver18(response.data[0].count_over18);
        setcountUnmarried(response.data[0].count_unmarried);
      }
...
<View style={{ marginTop:10,alignSelf:'center',}}>
      <Text style={styles.chartTitle}>Marital Status Analysis</Text>
      ...
      {data.length > 0 ? (
        <PieChart
          data={[
            {
              name: 'minors',
              count: parseInt(data[0].count_under18),
            ...
            {
              name: '18+ married',
              count: parseInt(data[0].count_over18),
              color: '#FFE933', // Color for MAM
              legendFontColor: '#7F7F7F',
              legendFontSize: 15,
            },
            {
                name: 'unmarried',
                count: parseInt(data[0].count_unmarried),
                        ...
                        <View style={styles.tableContainer}>
                            <View style={styles.tableHeader}>
                                ...
                                <View style={styles.tableRow}>
                                    <Text style={styles.tableCell}>Minors</Text>
                                    <Text style={styles.tableCell} >{countUnder18}</Text>
                                   
                                </View>
                                <View style={styles.tableRow}>
                                    <Text style={styles.tableCell}>Married 18+ year</Text>
                                    <Text style={styles.tableCell} >{countOver18}</Text>
                                   
                                </View>
                                <View style={styles.tableRow}>
                                    <Text style={styles.tableCell}>Unmarried</Text>
                                    <Text style={styles.tableCell} >{countUnmarried}</Text>
