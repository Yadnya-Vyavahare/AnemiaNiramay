...
const PersonalHeightGraph = () => {
 ...
 //const { params } = route;
  ...
  const fetchData = async () => {
    try {
      let response;
      response = await axios.get(`${config.API_BASE_URL}/HeightDifference`);
      
      // if(selectedKVarg!=null && selectedYear!=null){
      //   console.log(selectedKVarg+"12345"+selectedYear);
      //    response = await axios.get(http://192.168.137.1:8080/api/wtDifferenceNew?year=${selectedYear}&KVarg=${selectedKVarg});
   
      // }
      // else if(selectedKVarg!=null){
      //   console.log("helo");
      //    response = await axios.get(http://192.168.137.1:8080/api/wtDifferenceNew?KVarg=${selectedKVarg});
   
      // }
      // else if(selectedYear!=null){
      //    response = await axios.get(http://192.168.137.1:8080/api/wtDifferenceNew?year=${selectedYear});
   
      // }
      // else{
      //    response = await axios.get(http://192.168.137.1:8080/api/wtDifferenceNew);
  ...
  return (
    ...
    />
        ...
        <Text>No data available</Text>
          ...
          {clickedXValue !== null && (
        <>
          <Text>Clicked Data:</Text>
          <Text>First Name: {data[clickedXValue].FirstName}</Text>
          <Text>First Weight: {data[clickedXValue].FirstWt}</Text>
          <Text>Second Weight: {data[clickedXValue].SecondWt}</Text>
          <Text>Weight Difference: {data[clickedXValue].WeightDifference}</Text>
        </>
      )}
...
export default PersonalHeightGraph;