...
const AgeAnalysisComponent = () => {
  ...
  useEffect(() => {
    ...
    const fetchAgeData = async () => {
      try {
        ...
        response = await axios.get(`${config.API_BASE_URL}/anemicAnalysis?year=${selectedYear}&KVarg=${selectedKVarg}`);
      } else if (selectedKVarg !== null) {
        console.log('lolol')
        response = await axios.get(`${config.API_BASE_URL}/anemicAnalysis?KVarg=${selectedKVarg}`);
      } else if (selectedYear !== null) {
        response = await axios.get(`${config.API_BASE_URL}/anemicAnalysis?year=${selectedYear}`);
      } else {
        response = await axios.get(`${config.API_BASE_URL}/anemicAnalysis`);
      }
      ...
      if (!response || !response.data) {
        ...
        setcount(response.data[0].count_anemic);
        setcount10(response.data[0].count_normal);
      }
  ...
  return (
    ...
    <View style={{ marginTop:10,alignSelf:'center',}}>
      <Text style={styles.chartTitle}>Anemia Count</Text>
      ...
      {data.length > 0 ? (
        <PieChart
          data={[
            {
              name: 'Anemic',
              count: parseInt(data[0].count_anemic),
              color: '#ff0000', // Color for SAM
...
<View style={styles.table}>
                        ...
                        <View style={styles.tableContainer}>
                            <View style={styles.tableHeader}>
                                ...
                                <View style={styles.tableRow}>
                                    <Text style={styles.tableCell}>Anemic</Text>
